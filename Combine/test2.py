...
Original file is located at
    https://colab.research.google.com/drive/1G4Pyw667XbWJl-1j9ukGFloKSa6tOzuk
"""
...
class RealisticAttentionFSM:
    ...
    def update(self, yaw, pitch, EAR, blink_count, eye_closed, face_detected, movement, hand_output):
        ...
        # 산만 감지: yaw/pitch 과도 변화 ≥ 3회/10초 (얼굴 인식 성공 시에만 기록)
        if face_detected:
            self.yaw_pitch_history.append((now, yaw, pitch))
            self.yaw_pitch_history = [
                entry for entry in self.yaw_pitch_history if (now - entry[0]).total_seconds() <= 10
            ]
