...
class YoloTRT:
    def __init__(self, engine_path):
        ...
        output_shape = (1, 25200 * 7)  # output format: num, followed by flattened (x, y, w, h, conf, cls_id, score)
        ...
        self.LEN_ONE_RESULT = 7  # YOLOv5n 출력 7개
    ...
    # PostProcess 함수 내부에서 사용되는 NonMaxSuppression
    ...
    # NonMaxSuppression 함수 내부에서 사용되는 xywh2xyxy
    # Yolo의 중심 좌표 기반 box를 x1, y1, x2, y2로 바꾸는 함수
    ...
    # NonMaxSuppression 함수 내부에서 사용되는 bbox_iou
    # IOU 계산 함수
    ...
    def get_held_status(self, boxes, class_ids, hand_class_id=7, iou_thresh=0.2):
        results = {}
        hand_boxes = [box for box, cid in zip(boxes, class_ids) if int(cid) == hand_class_id]

        for box, cid in zip(boxes, class_ids):
            cid = int(cid)
            if cid == hand_class_id:
                continue  # 손 자체는 skip

            # IOU 비교: 현재 box vs 모든 손 box
            is_held = any(self.compute_iou(box, hand_box) > iou_thresh for hand_box in hand_boxes)

            if cid not in results:
                results[cid] = []

            results[cid].append((box, is_held))

        return results

    def determine_state(self, held_map):
        found_states = set()

        for cid, entries in held_map.items():
            for _, is_held in entries:
                if is_held:
        ...
        if 2 in found_states:
            return 2  # Study
        elif 0 in found_states:
            return 0  # Moderate
        else:
            return 0  # 아무것도 없을 경우 Moderate
    ...
    def hand_code(self, frame):
        output, t = self.Inference(frame)
        boxes, scores, class_ids = self.PostProcess(output, frame.shape[0], frame.shape[1])
            
        held_map = self.get_held_status(boxes, class_ids)
        state = self.determine_state(held_map)

        for cid, entries in held_map.items():
            class_name = self.class_names[cid]
            for box, is_held in entries:
                x1, y1, x2, y2 = map(int, box)
                label = f"{class_name} {'[Held]' if is_held else '[Free]'}"
    ...
    def run_camera_inference(self):
        ...
        while True:
            ...
            frame, hand_state = self.hand_code(frame)
            # hand_state information
            # 0 : Moderate
            # 1 : Play
            # 2 : Study
