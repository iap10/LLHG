...
class YoloTRT:
    ...
    def compute_iou_matrix(self, A, B):
        A = A[:, None, :]  # (N, 1, 4)
        B = B[None, :, :]  # (1, M, 4)
    ...
    def held_status_vectorized(self, boxes, class_ids, hand_class_id=7, iou_thresh=0.2):
        ...
        if len(hand_boxes) == 0 or len(object_boxes) == 0:
            return object_boxes, object_ids, np.zeros(len(object_boxes), dtype=bool)

        iou_matrix = self.compute_iou_matrix(object_boxes, hand_boxes)
        held = (iou_matrix > iou_thresh).any(axis=1)
